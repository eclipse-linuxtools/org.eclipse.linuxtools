...
public class OprofileSAXHandler extends DefaultHandler {
	...
	// As per SAX spec, SAX parsers are allowed to split character data into as many
	// chunks as
	// they please, and they can split the text at whichever boundaries they want.
	// In order to
	...
	private static XMLProcessor getProcessor(String type) {
		...
		if (handlerClass != null) {
			try {
				processor = (XMLProcessor) handlerClass.getDeclaredConstructor().newInstance();
	...
	/**
	 * Removes the current XMLProcessor and installs the previous processor. NOTE:
