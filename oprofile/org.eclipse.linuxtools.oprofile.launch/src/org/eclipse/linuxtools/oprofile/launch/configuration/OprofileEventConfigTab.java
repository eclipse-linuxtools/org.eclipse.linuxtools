...
public class OprofileEventConfigTab extends AbstractLaunchConfigurationTab {
	...
	public boolean isValid(ILaunchConfiguration config) {
		...
		OprofileCounter[] counters = new OprofileCounter[Oprofile
				.getNumberOfCounters()];
		for (int i = 0; i < counters.length; i++) {
			...
			if (counters[i].getEnabled()) {
				...
				if (!Oprofile.checkEvent(counters[i].getNumber(), counters[i]
						.getEvent().getNumber(), counters[i].getEvent()
						.getUnitMask().getMaskValue())) {
					valid = false;
					break;
				}
			}
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy config) {
		...
		config.setAttribute(OprofileLaunchPlugin.ATTR_USE_DEFAULT_EVENT,
				useDefault);
	}
	...
	public void createControl(Composite parent) {
		...
		data = new GridData();
		data.widthHint = _counterCombo.computeSize(SWT.DEFAULT, SWT.DEFAULT).x + 30;
		...
		_eventList = new ListViewer(left, SWT.SINGLE | SWT.V_SCROLL
				| SWT.H_SCROLL | SWT.BORDER);
		// Fix layout data (height, width) after we construct right-hand
		// composite.
		// data = new GridData(GridData.FILL_BOTH);
		...
		_eventList.getList().addMouseTrackListener(new MouseTrackAdapter() {
			...
			// slowly does this really work. Must be a way to get Enter/Leave
			// events...
		...
		_profileKernelButton
				.setText(OprofileLaunchMessages
						.getString("tab.event.counterSettings.profileKernel.check.text")); //$NON-NLS-1$
		...
		_profileUserspaceButton.setText(OprofileLaunchMessages
				.getString("tab.event.counterSettings.profileUser.check.text")); //$NON-NLS-1$
		...
		_countText.addVerifyListener(new VerifyListener() {
			public void verifyText(VerifyEvent ve) {
				// Only allow numbers and non-text events (ie. backspace and
				// delete)
				// SUCK FIXME: i18n?
				try {
					// An event which doesn't contain text (backspace and
					// delete)
	...
	private void _handleProfileUserToggle() {
		if (_currentCounter() != null) {
			_currentCounter().setProfileUser(
					_profileUserspaceButton.getSelection());
			updateLaunchConfigurationDialog();
		}
	}
	...
	private void _handleProfileKernelToggle() {
		if (_currentCounter() != null) {
			_currentCounter().setProfileKernel(
					_profileKernelButton.getSelection());
			updateLaunchConfigurationDialog();
		}
	}
	...
	private void _handleEnabledToggle() {
		...
		OprofileCounter c = (OprofileCounter) _counterCombo
				.getData(_counterCombo.getText());
	...
	// of the given event. If it is TEMPORARY, then the next call to
	// resetEventDesc
	// will wipe this out. If it is not TEMPORARY, then the next call to
	// resetEventDesc
