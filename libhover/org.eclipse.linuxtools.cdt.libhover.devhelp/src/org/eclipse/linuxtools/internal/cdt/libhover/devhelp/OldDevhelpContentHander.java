...
public class OldDevhelpContentHander implements IDevhelpContentHandler {
	...
	public OldDevhelpContentHander(Map<String, String> funcs) {
		this.funcs = funcs;
	}
		...
		if (begin) {
			...
			if (!descStart) {
				if ("SPAN".equals(localName)) { //$NON-NLS-1$
					...
					if (returnValue == null && type != null && type.equals("returnvalue")) { //$NON-NLS-1$
						returnType = true;
					}
				} else if ("PRE".equals(localName)) { //$NON-NLS-1$
					String type = a.getValue("class"); //$NON-NLS-1$
	...
	public void characters(char[] ch, int start, int length) {
		if (begin) {
			if (returnType) {
				...
				String temp = new String(ch).toString().trim();
				boolean completed = false;
				...
				String parmData = new String(ch).toString().trim();
