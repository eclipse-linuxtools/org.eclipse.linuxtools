	...
	public void testNullDefinition() {
		...
		for (int i = 0; i < markers.length; i++) {
			...
			if ((marker.getAttribute(IMarker.CHAR_START, 0) == 70) &&
				...
				assertEquals("No value name after define.", marker.getAttribute(IMarker.MESSAGE, ""));
				fail = false;
			}
		}
		if (fail)
	...
	public void testNonLetterDefinitionName() {
		...
		for (int i = 0; i < markers.length; i++) {
			...
			if ((marker.getAttribute(IMarker.CHAR_START, 0) == 84) &&
				...
				assertEquals("Definition lvalue must begin with a letter or an underscore.", marker.getAttribute(IMarker.MESSAGE, ""));
				fail = false;
			}
		}
		if (fail)
