...
public class Aggregator {
	...
	public static File aggregate(String gprof2use, String binaryFile, java.util.List<String> gmons, File directory) {
		...
		try {
			...
			int ret = p.waitFor();
			if (ret != 0) {
				errorMessage = "Error during STGprof aggregation\n";
		...
		if (errorMessage != null) {
			...
			PlatformUI.getWorkbench().getDisplay().asyncExec(
					new Runnable() {
						public void run() {
							MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "STGprof error", finalErrorMessage);
						}
					}
			);
			return null;
